...
(() => {
  ...
  function sampleGames(games, team, venue, opponent, competition="", season="") {
    return games.filter(g => (g.home === team || g.away === team) && (!venue || (venue === "home" ? g.home === team : g.away === team)) && (!opponent || (g.home === team ? g.away : g.home) === opponent) && (!competition || g.competition === competition) && (!season || g.season === season));
  }
  const seasonFor = date => {const d=new Date(date);if(Number.isNaN(d.getTime()))return "Unknown";const start=d.getUTCMonth()>=6?d.getUTCFullYear():d.getUTCFullYear()-1;return `${start}-${String(start+1).slice(-2)}`;};
  ...
  function evidenceCards(rows, key) {
    ...
    const make=polarity=>subset.filter(e=>e.polarity===polarity).map(e=>{
      ...
      return `<li><a href="${escape(link.toString())}" target="_blank" rel="noopener noreferrer">${escape(e.possession_note)} · ${e.start_seconds}s</a> <small>game ${e.game_id}${e.lineup_label?" · lineup: "+escape(e.lineup_label):""}</small></li>`;
    }).join("");
  ...
  async function open() {
    ...
    const mapGame=row=>({...row.payload?.ui,_dbId:row.id,sourceUrl:row.source_url,provider:row.provider,competition:row.competition||row.payload?.ui?.comp||"Unknown",season:seasonFor(row.game_date||row.payload?.ui?.date)});
    let saved=workspace.games.map(mapGame).filter(g=>g.home&&g.away);
    ...
    content.innerHTML=`<div class="coachControls"><label>Scout team <select id="coachTeam">${options(teams)}</select></label><label>Competition <select id="coachCompetition"></select></label><label>Season <select id="coachSeason"></select></label><label>Venue <select id="coachVenue"><option value="">All</option><option value="home">Home</option><option value="away">Away</option></select></label><label>Opponent <select id="coachOpponent"><option value="">All opponents</option></select></label><label>Video lineup tag <select id="coachLineup"><option value="">All clips</option></select></label><label>Player role <select id="coachRole"><option value="">All</option><option value="starter">Starter</option><option value="bench">Bench</option></select></label><label>Minimum minutes <input id="coachMinutes" type="number" min="0" max="60" value="0"></label></div><div id="coachSummary"></div><details class="coachOps"><summary>Source coverage, revisions and analyst review</summary><div id="coachOperations"></div></details>`;
    content.querySelector("#coachTeam").value=defaultTeam;
    const refreshCompetition=()=>{const team=content.querySelector("#coachTeam").value,comps=[...new Set(saved.filter(g=>g.home===team||g.away===team).map(g=>g.competition))].sort();content.querySelector("#coachCompetition").innerHTML=options(comps);};
    const refreshSeason=()=>{const team=content.querySelector("#coachTeam").value,comp=content.querySelector("#coachCompetition").value,seasons=[...new Set(saved.filter(g=>(g.home===team||g.away===team)&&g.competition===comp).map(g=>g.season))].sort().reverse();content.querySelector("#coachSeason").innerHTML=options(seasons);};
    const refreshOpponent=()=>{const team=content.querySelector("#coachTeam").value,comp=content.querySelector("#coachCompetition").value,season=content.querySelector("#coachSeason").value,opps=[...new Set(saved.filter(g=>(g.home===team||g.away===team)&&g.competition===comp&&g.season===season).map(g=>g.home===team?g.away:g.home))].sort();content.querySelector("#coachOpponent").innerHTML='<option value="">All opponents</option>'+options(opps);};
    const refreshLineup=()=>{const team=content.querySelector("#coachTeam").value,comp=content.querySelector("#coachCompetition").value,season=content.querySelector("#coachSeason").value,ids=new Set(sampleGames(saved,team,"","",comp,season).map(g=>g._dbId));const tags=[...new Set(extra.evidence.filter(e=>ids.has(e.game_id)).map(e=>e.lineup_label).filter(Boolean))].sort();content.querySelector("#coachLineup").innerHTML='<option value="">All clips</option>'+options(tags);};
    function draw(){
      const team=content.querySelector("#coachTeam").value,venue=content.querySelector("#coachVenue").value,opp=content.querySelector("#coachOpponent").value,competition=content.querySelector("#coachCompetition").value,season=content.querySelector("#coachSeason").value;
      const sample=sampleGames(saved,team,venue,opp,competition,season), rows=sample.map(g=>gameRow(g,team)), selectedIds=new Set(sample.map(g=>g._dbId));
      const lineup=content.querySelector("#coachLineup").value,evidence=extra.evidence.filter(e=>selectedIds.has(e.game_id)&&(!lineup||e.lineup_label===lineup));
      ...
      content.querySelector("#coachSummary").innerHTML=`<div class="insight">${sample.length} saved games · ${escape(competition||"No competition")} · ${escape(season||"No season")} · ${period} · ${escape(venue||"all venues")} · ${escape(opp||"all opponents")}. Only stored official imports are included.</div><div class="coachClaims">${cards}</div><div class="metricNote">Formula set: eFG% = (FGM + 0.5 × 3PM) / FGA; TOV% = TOV / (FGA + 0.44 × FTA + TOV); ORB% = OREB / (OREB + opponent DREB). Game values are calculated by the deterministic import pipeline. Missing values are excluded from each metric's displayed sample.</div>${lineup?`<div class="insight warning">Lineup tag “${escape(lineup)}” filters linked clips only. Numeric values above still describe all selected games.</div>`:""}<h3>Minutes and role · ${escape(team)}</h3><table class="stats"><tr><th>Player</th><th>Games</th><th>Average minutes</th><th>Starts</th><th>Points / game</th></tr>${players.map(p=>`<tr><td>${escape(p.name)}</td><td>${p.games}</td><td>${Math.round(p.minutes/p.games*10)/10}</td><td>${p.starts}</td><td>${Math.round(p.points/p.games*10)/10}</td></tr>`).join("")||'<tr><td colspan="5">No player rows match these filters.</td></tr>'}</table><div class="insight warning">Lineup combinations and on/off impact require substitution-level events or tagged video. A box score supplies starters and minutes, not five-player lineup possessions.</div>`;
    }
    function drawOps(){
      ...
      content.querySelector("#importMissing").onclick=async()=>{const errors=[];for(const row of pending){try{status.textContent="Importing "+row.home_team+" – "+row.away_team;await data.importOfficialGame(row.source_url);imported.add(sourceKey(row.source_url));}catch(err){errors.push(row.home_team+" – "+row.away_team+": "+err.message);}}workspace=await data.workspace();saved=workspace.games.map(mapGame).filter(g=>g.home&&g.away);extra=await data.coachWorkspace(workspace.club.id,workspace.games.map(g=>g.id));refreshCompetition();refreshSeason();refreshOpponent();refreshLineup();drawOps();draw();content.querySelector("#coachStatus").textContent=errors.length?errors.join(" · "):"Import batch complete.";};
      ...
      content.querySelector("#saveEvidence").onclick=async()=>{try{const val=id=>content.querySelector("#"+id).value.trim(),url=val("evidenceUrl"),start=Number(val("evidenceStart")),end=val("evidenceEnd");if(!sourceHost(url)&&!/^https:\/\//.test(url))throw new Error("Use an authorized HTTPS video URL.");if(!Number.isInteger(start)||start<0||!val("evidenceStart"))throw new Error("Enter clip start in seconds.");if(val("evidenceNote").length<5)throw new Error("Describe this possession.");await data.addEvidence({club_id:workspace.club.id,game_id:Number(val("evidenceGame")),claim_key:val("evidenceClaim"),polarity:val("evidencePolarity"),video_url:url,start_seconds:start,end_seconds:end?Number(end):null,lineup_label:val("evidenceLineup")||null,possession_note:val("evidenceNote")});extra=await data.coachWorkspace(workspace.club.id,workspace.games.map(g=>g.id));drawOps();refreshLineup();draw();}catch(err){status.textContent=err.message;}};
    }
    content.querySelector("#coachTeam").onchange=()=>{refreshCompetition();refreshSeason();refreshOpponent();refreshLineup();draw();};
    content.querySelector("#coachCompetition").onchange=()=>{refreshSeason();refreshOpponent();refreshLineup();draw();};
    content.querySelector("#coachSeason").onchange=()=>{refreshOpponent();refreshLineup();draw();};
    ["coachVenue","coachOpponent","coachLineup","coachRole","coachMinutes"].forEach(id=>content.querySelector("#"+id).onchange=draw);
    refreshCompetition();refreshSeason();refreshOpponent();refreshLineup();draw();drawOps();
  }
