...
// Season intelligence is based on persisted imports only. Bundled demonstrations
// and browser-local CSVs must never be presented as official season history.
function officialSeasonGames(){
  return Object.entries(games).filter(([key,g])=>key.startsWith("db_")&&g?._dbId&&g.home&&g.away)
    .map(([,g])=>g).sort((a,b)=>{
      const date=g=>{const t=Date.parse(g.date||"");return Number.isFinite(t)?t:0;};
      return date(a)-date(b)||String(a._dbId).localeCompare(String(b._dbId));
    });
}
function localTeamGames(team){return officialSeasonGames().filter(g=>g.home===team||g.away===team);}
function oneDecimal(n){return Math.round(n*10)/10}
function buildSeasonMemory(){
  const all=officialSeasonGames();
...
async function openSeasonMemory(){
  ...
  document.body.appendChild(modal);modal.querySelector(".modalX").onclick=()=>modal.remove();modal.onclick=e=>{if(e.target===modal)modal.remove();};
  modal.querySelectorAll("[data-memory-team]").forEach(btn=>btn.onclick=()=>{const t=memory.teams.find(x=>x.team===btn.dataset.memoryTeam);const changes=[["Scoring",t.delta.pf,false],["Shot efficiency",t.delta.efg,false],["Ball security",t.delta.tov,true],["Offensive rebounding",t.delta.orb,false],["Net Rating",t.delta.net,false]].filter(x=>x[1]!=null).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1]));modal.querySelector("#memoryIntel").innerHTML=`<section class="card takeaways"><div class="sectionhead"><h3>COACH INTELLIGENCE · ${htmlEsc(t.team)}</h3><small>${t.games} GAME MEMORY</small></div>${changes.length?`<div class="findings">${changes.slice(0,3).map((x,i)=>`<div class="finding"><div class="num">${i+1}</div><b>${x[0]}</b><p>${x[1]>0?"+":""}${x[1]} vs previous five-game sample</p><em>→ ${x[2]?"Check whether the reduction is stable and what possessions drove it.":"Review the possessions behind this change before treating it as a tactical tendency."}</em></div>`).join("")}</div>`:'<div class="insight warning">Ten stored games are needed for a full five-versus-five comparison.</div>'}<div class="videoList"><div class="videoItem"><span>1</span>Verify the largest statistical change on full-game video.</div><div class="videoItem"><span>2</span>Separate opponent/context effects from repeatable team behavior.</div><div class="videoItem"><span>3</span>Use the next game to test whether the trend persists.</div></div></section>`;});
}
function buildPlayerMemory(){
  const all=officialSeasonGames(),map=new Map();
  all.forEach(g=>[["home",g.home],["away",g.away]].forEach(([side,team])=>(g.players?.[side]||[]).forEach(p=>{const key=team+"::"+p.name;if(!map.has(key))map.set(key,{team,name:p.name,games:[]});map.get(key).games.push({date:g.date||"—",opponent:g[side==="home"?"away":"home"],minutes:Number(p.minutes),points:Number(p.points),rebounds:Number(p.rebounds),ast:Number(p.ast),tov:Number(p.tov),efg:Number(p.efg),ts:Number(p.ts),usage:Number(p.play_end_share),impact:Number(p.box_impact_per_40),threeRate:Number(p.three_pa_rate)});})));
  const avg=(a,k)=>{const v=a.map(x=>x[k]).filter(x=>x!==null&&x!==undefined&&x!=="").map(Number).filter(Number.isFinite);return v.length?oneDecimal(v.reduce((s,n)=>s+n,0)/v.length):null;};
  return [...map.values()].map(x=>{x.games.sort((a,b)=>(Date.parse(a.date)||0)-(Date.parse(b.date)||0));const recent=x.games.slice(-4),prev=x.games.slice(-8,-4),delta=k=>{const a=avg(recent,k),b=avg(prev,k);return a!=null&&b!=null?oneDecimal(a-b):null;};return {...x,recent,prev,avg:{minutes:avg(recent,"minutes"),points:avg(recent,"points"),rebounds:avg(recent,"rebounds"),ast:avg(recent,"ast"),tov:avg(recent,"tov"),efg:avg(recent,"efg"),ts:avg(recent,"ts"),usage:avg(recent,"usage"),impact:avg(recent,"impact")},delta:{minutes:delta("minutes"),points:delta("points"),efg:delta("efg"),ts:delta("ts"),usage:delta("usage"),impact:delta("impact"),tov:delta("tov")}};}).sort((a,b)=>b.games.length-a.games.length||a.name.localeCompare(b.name));
}
async function openPlayerMemory(){
  let syncError="";if(window.CourtIQData?.isSignedIn()){try{await syncProductData();}catch(e){syncError=e.message;}}
  const players=buildPlayerMemory(),modal=document.createElement("div");modal.className="modal";
  const signal=p=>{if(p.recent.length<4||p.prev.length<4)return null;const d=p.delta,vals=[["Scoring",d.points],["eFG%",d.efg],["TS%",d.ts],["Involvement",d.usage],["Impact/40",d.impact],["Minutes",d.minutes]].filter(x=>x[1]!=null).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1]));return vals[0]||null;};
  const cards=players.map(p=>{const s=signal(p),alert=s&&Math.abs(s[1])>=5;return `<div class="card box"><small>${htmlEsc(p.team)}</small><h3>${htmlEsc(p.name)}</h3><div class="reportKpis"><div><small>GAMES</small><b>${p.games.length}</b></div><div><small>MIN · L4</small><b>${p.avg.minutes??"—"}</b></div><div><small>PTS · L4</small><b>${p.avg.points??"—"}</b></div><div><small>eFG%</small><b>${p.avg.efg??"—"}${p.avg.efg!=null?"%":""}</b></div><div><small>INVOLVEMENT</small><b>${p.avg.usage??"—"}${p.avg.usage!=null?"%":""}</b></div><div><small>IMPACT/40</small><b>${p.avg.impact??"—"}</b></div></div>${s?`<div class="insight ${alert?"warning":""}"><b>${alert?"PROFILE CHANGE DETECTED":"PROFILE WATCH"}</b> · ${s[0]} ${s[1]>0?"+":""}${s[1]} vs previous 4-game sample.</div>`:'<div class="metricNote">Need 8 stored player games for a full 4-vs-4 comparison.</div>'}<button class="runImport" data-player-memory="${htmlEsc(p.team+"::"+p.name)}">PLAYER INTELLIGENCE</button></div>`;}).join("");
  modal.innerHTML=`<div class="modalCard compareModal gameLibrary"><button class="modalX">×</button><small class="eyebrow">COURTIQ · PLAYER MEMORY ENGINE</small><h2>Player Memory</h2><p>Official player box scores become a season-long memory. CourtIQ compares the last four games with the previous four and flags meaningful profile changes without inventing missing data.</p><div class="libraryTop"><div><small>PLAYERS REMEMBERED</small><b>${players.length}</b></div><div><small>COMPARISON</small><b>LAST 4 vs PREVIOUS 4</b></div><div><small>SOURCE</small><b>OFFICIAL PLAYER BOX SCORES</b></div></div><div class="gameLibraryGrid">${cards||'<div class="productEmpty"><b>NO PLAYER MEMORY YET</b><span>Import official games containing player rows to build player memory.</span></div>'}</div>${syncError?`<div class="insight warning">DATABASE · ${htmlEsc(syncError)}</div>`:""}<div id="playerMemoryIntel"></div></div>`;
  document.body.appendChild(modal);modal.querySelector(".modalX").onclick=()=>modal.remove();modal.onclick=e=>{if(e.target===modal)modal.remove();};
...
function avgFinite(rows,key){const v=rows.map(x=>x[key]).filter(x=>x!==null&&x!==undefined&&x!=="").map(Number).filter(Number.isFinite);return v.length?oneDecimal(v.reduce((a,b)=>a+b,0)/v.length):null;}
function seasonTrendPanel(G){
  ...
  const metric=(key,label,suffix="")=>{const now=avgFinite(recent,key),before=avgFinite(prev,key),delta=now!=null&&before!=null?oneDecimal(now-before):null;return `<div class="card kpi"><label>${label} · LAST ${recent.length}</label><b>${now??"—"}${now!=null?suffix:""}</b><small>${delta==null?"Need 10 stored games for 5-vs-5":(delta>0?"+":"")+delta+" vs previous sample"}</small></div>`;};
  return `<section class="card takeaways seasonIntel"><div class="sectionhead"><h3><span>↗</span> SEASON TRENDS · ${htmlEsc(team)}</h3><small>${rows.length} SAVED GAME${rows.length===1?"":"S"}</small></div><div class="kpis">${metric("pf","Points","")}${metric("efg","eFG%","%")}${metric("tov","TOV%","%")}${metric("net","Net Rating","")}${metric("pace","Pace","")}</div><div class="metricNote">Trend calculations use only official games already stored in CourtIQ. Small samples are labeled; no missing games are estimated.</div></section>`;
}
...
async function openCoachDashboard(){
 ...
 try{
  ...
  const intel=await window.CourtIQData.coachIntelligence(comp,opp),mem=intel?.team_memory||{},claims=intel?.evidence_claims||[];
  const verified=claims||[], trends=verified.slice(0,3), playerSignals=(intel?.players||[]).filter(x=>["ROLE_CHANGE","TREND"].includes(x.flag)).slice(0,5), quality=intel?.data_quality||{};
  ...
  <h3>Player Signals</h3>${playerSignals.length?playerSignals.map(x=>`<div class="schema"><b>${htmlEsc(x.player_name)} · ${x.flag}</b><br>Season: ${x.summary?.ppg??"—"} PPG / ${x.summary?.mpg??"—"} MPG · Last 5: ${x.last5?.ppg??"—"} PPG / ${x.last5?.mpg??"—"} MPG · Confidence ${Math.round(Number(x.confidence||0)*100)}%</div>`).join(""):'<div class="schema"><b>NO VERIFIED PLAYER TREND</b><br>CourtIQ will not infer a role change from an insufficient sample.</div>'}
  <h3>Data Quality</h3><div class="schema"><b>${quality.status||"INSUFFICIENT_DATA"}</b> · ${quality.passed||0}/${quality.checks||0} checks passed${quality.failed?" · "+quality.failed+" failed":""}</div>
  <h3>Matchups / Trends / Outliers</h3><div class="schema">${verified.length||playerSignals.length?"Evidence-backed intelligence is active.":"INSUFFICIENT DATA · CourtIQ will not invent conclusions."}</div>
...
async function openOpponentScout(){
 ...
 const matchups=sig.slice(0,3).map((x,i)=>({n:i+1,player:x.player_name,flag:x.flag,confidence:Math.round(Number(x.confidence||0)*100),season:x.summary?.ppg,last5:x.last5?.ppg}));
 const videos=claims.flatMap(x=>Array.isArray(x.video)?x.video:[]).slice(0,6);
 const prep={team:s.team,competition:s.competition,quality:q,profile:p,tendencies:tend,matchups,alerts:sig.slice(0,5),evidence:claims.length?claims.slice(0,5):ev.slice(0,5),videos,watch:watch.slice(0,3),games:s.games};
 const openPrep=()=>{const w=window.open("","_blank");const esc=x=>String(x??"").replace(/[&<>"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]));w.document.write(`<!doctype html><meta charset="utf-8"><title>CourtIQ Game Prep · ${esc(prep.team)}</title><style>@page{size:A4;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;margin:0;color:#111;background:#fff;font-size:11px}header{border-bottom:3px solid #111;padding-bottom:8px;margin-bottom:10px}h1{margin:0;font-size:24px}h2{font-size:13px;margin:10px 0 5px;text-transform:uppercase;letter-spacing:.06em}.meta{display:flex;justify-content:space-between}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.card{border:1px solid #bbb;border-radius:7px;padding:7px;break-inside:avoid}.item{border-top:1px solid #ddd;padding:5px 0}.item:first-child{border:0}.tag{font-weight:700}.warn{font-weight:700}.foot{margin-top:8px;border-top:1px solid #aaa;padding-top:5px;font-size:9px}@media print{button{display:none}}</style><header><small>COURTIQ V2 · ONE-PAGE GAME PREP</small><h1>${esc(prep.team)}</h1><div class="meta"><span>${esc(prep.competition)}</span><b>DATA QUALITY: ${esc(prep.quality?.status||"INSUFFICIENT_DATA")}</b></div></header><div class="grid"><section><h2>3 Opponent Tendencies</h2><div class="card">${prep.tendencies.length?prep.tendencies.map((x,i)=>`<div class="item"><b>${i+1}. ${esc(x.area)}</b> · Last 5 ${esc(x.last5)} vs Season ${esc(x.season)} · Δ ${Number(x.delta)>=0?"+":""}${esc(x.delta)}<br><small>Sample ${esc(x.sample_games)} games</small></div>`).join(""):"<b>INSUFFICIENT DATA</b>"}</div><h2>3 Matchups</h2><div class="card">${prep.matchups.length?prep.matchups.map(x=>`<div class="item"><b>${x.n}. ${esc(x.player)}</b> · ${esc(x.flag)}<br>Season ${esc(x.season??"—")} PPG → Last 5 ${esc(x.last5??"—")} · Confidence ${x.confidence}%</div>`).join(""):"<b>INSUFFICIENT DATA</b><br>Player matchup assignment requires verified player evidence."}</div><h2>Player Alerts</h2><div class="card">${prep.alerts.length?prep.alerts.map(x=>`<div class="item"><span class="tag">${esc(x.player_name)} · ${esc(x.flag)}</span> · ${Math.round(Number(x.confidence||0)*100)}%</div>`).join(""):"No verified player alerts."}</div></section><section><h2>Team Profile</h2><div class="card">Pace <b>${esc(p.pace??"—")}</b> · eFG% <b>${esc(p.efg??"—")}</b> · TOV% <b>${esc(p.tov_pct??"—")}</b> · ORB% <b>${esc(p.orb_pct??"—")}</b> · FTr <b>${esc(p.ftr??"—")}</b></div><h2>Evidence</h2><div class="card">${prep.evidence.length?prep.evidence.map(x=>`<div class="item">${x.claim?"<b>"+esc(x.claim)+"</b> · "+esc(x.sample_games)+" games · "+Math.round(Number(x.confidence||0)*100)+"%":"Game #"+esc(x.game_id)+" · "+esc(x.date||"—")+" vs "+esc(x.opponent||"—")}</div>`).join(""):"<b>INSUFFICIENT DATA</b>"}</div><h2>Video Evidence</h2><div class="card">${prep.videos.length?prep.videos.map((v,i)=>`<div class="item">${i+1}. ${esc(typeof v==="string"?v:(v.label||v.url||"Video evidence"))}</div>`).join(""):"VIDEO EVIDENCE PENDING"}</div><h2>WHAT TO WATCH</h2><div class="card">${prep.watch.length?prep.watch.map((x,i)=>`<div class="item"><b>${i+1}.</b> ${esc(x)}</div>`).join(""):"<b>INSUFFICIENT DATA</b>"}</div></section></div><div class="foot">CourtIQ · Data → Evidence → Decision support. Tactical decisions remain with the coaching staff. · ${esc(prep.games)} accessible games.</div><button onclick="print()">PRINT / SAVE PDF</button>`);w.document.close()};
 ...
 <h3>Video</h3><div class="schema">${videos.length?videos.map((v,i)=>(i+1)+". "+htmlEsc(typeof v==="string"?v:(v.label||v.url||"Video evidence"))).join("<br>"):"VIDEO EVIDENCE PENDING"}</div>
 <h3>What to Watch</h3><div class="schema">${watch.length?watch.map(x=>"• "+htmlEsc(x)).join("<br>"):"INSUFFICIENT DATA · no unsupported tactical recommendation generated."}</div><button id="osGamePrep" class="runImport">OPEN ONE-PAGE GAME PREP</button>`;out.querySelector("#osGamePrep").onclick=openPrep;
