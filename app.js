...
function buildPlayerMemory(){
  const all=productGameEntries().map(([,g])=>g).filter(Boolean),map=new Map();
  ...
  const avg=(a,k)=>{const v=a.map(x=>Number(x[k])).filter(Number.isFinite);return v.length?oneDecimal(v.reduce((s,n)=>s+n,0)/v.length):null;};
  return [...map.values()].map(x=>{x.games.sort((a,b)=>String(a.date).localeCompare(String(b.date)));const recent=x.games.slice(-4),prev=x.games.slice(-8,-4),delta=k=>{const a=avg(recent,k),b=avg(prev,k);return a!=null&&b!=null?oneDecimal(a-b):null;};return {...x,recent,prev,avg:{minutes:avg(recent,"minutes"),points:avg(recent,"points"),rebounds:avg(recent,"rebounds"),ast:avg(recent,"ast"),tov:avg(recent,"tov"),efg:avg(recent,"efg"),ts:avg(recent,"ts"),usage:avg(recent,"usage"),impact:avg(recent,"impact")},delta:{minutes:delta("minutes"),points:delta("points"),efg:delta("efg"),ts:delta("ts"),usage:delta("usage"),impact:delta("impact"),tov:delta("tov")}};}).sort((a,b)=>b.games.length-a.games.length||a.name.localeCompare(b.name));
}
async function openPlayerMemory(){
  let syncError="";if(window.CourtIQData?.isSignedIn()){try{await syncProductData();}catch(e){syncError=e.message;}}
  const players=buildPlayerMemory(),modal=document.createElement("div");modal.className="modal";
  const signal=p=>{const d=p.delta,vals=[["Scoring",d.points],["eFG%",d.efg],["TS%",d.ts],["Involvement",d.usage],["Impact/40",d.impact],["Minutes",d.minutes]].filter(x=>x[1]!=null).sort((a,b)=>Math.abs(b[1])-Math.abs(a[1]));return vals[0]||null;};
  const cards=players.map(p=>{const s=signal(p),alert=s&&Math.abs(s[1])>=5;return `<div class="card box"><small>${htmlEsc(p.team)}</small><h3>${htmlEsc(p.name)}</h3><div class="reportKpis"><div><small>GAMES</small><b>${p.games.length}</b></div><div><small>MIN · L4</small><b>${p.avg.minutes??"—"}</b></div><div><small>PTS · L4</small><b>${p.avg.points??"—"}</b></div><div><small>eFG%</small><b>${p.avg.efg??"—"}${p.avg.efg!=null?"%":""}</b></div><div><small>INVOLVEMENT</small><b>${p.avg.usage??"—"}${p.avg.usage!=null?"%":""}</b></div><div><small>IMPACT/40</small><b>${p.avg.impact??"—"}</b></div></div>${s?`<div class="insight ${alert?"warning":""}"><b>${alert?"PROFILE CHANGE DETECTED":"PROFILE WATCH"}</b> · ${s[0]} ${s[1]>0?"+":""}${s[1]} vs previous 4-game sample.</div>`:'<div class="metricNote">Need 5+ stored player games to begin comparison; 8 gives a full 4-vs-4 baseline.</div>'}<button class="runImport" data-player-memory="${htmlEsc(p.team+"::"+p.name)}">PLAYER INTELLIGENCE</button></div>`;}).join("");
