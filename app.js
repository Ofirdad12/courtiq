...
function openUrlImport(){
  const modal=document.createElement("div"); modal.className="modal";
  modal.innerHTML='<div class="modalCard"><button class="modalX">×</button><small class="eyebrow">PRIMARY PILOT WORKFLOW</small><h2>Paste Box Score Link</h2><p>Paste an official IBBA match URL. CourtIQ will fetch, validate, calculate and save the game.</p><div class="importFields"><input id="boxUrl" placeholder="https://ibasketball.co.il/match/778678/"><input id="apiUrl" placeholder="CourtIQ API URL" value="'+API_BASE()+'"></div><div class="schema">Supported now: official IBBA match links. FIBA URL ingestion is next. CSV remains a fallback.</div><div id="urlStatus" class="impStatus"></div><button id="runUrl" class="runImport">ANALYZE GAME</button></div>';
  document.body.appendChild(modal);
  modal.querySelector(".modalX").onclick=()=>modal.remove();
  modal.querySelector("#runUrl").onclick=async()=>{
    const status=modal.querySelector("#urlStatus"),base=modal.querySelector("#apiUrl").value.trim().replace(/\/$/,""),url=modal.querySelector("#boxUrl").value.trim();
    try{
      if(!base) throw new Error("CourtIQ backend is not connected yet. Connect the pilot API first.");
      localStorage.setItem("courtiq_api_base",base); status.textContent="Importing official box score…";
      const res=await fetch(base+"/api/import-url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({url})});
      const body=await res.json(); if(!res.ok) throw new Error(body.detail||"Import failed");
      games.url=apiGameToUi(body); localStorage.setItem("courtiq_url_game",JSON.stringify(games.url)); active="url"; modal.remove(); render(); window.scrollTo(0,0);
...
function openAccount(){
  ...
  modal.innerHTML=`<div class="modalCard accountModal"><button class="modalX">×</button>
    <small class="eyebrow">COURTIQ · SECURE CLUB ACCESS</small><h2>${signed?"Club Account":"Sign in to CourtIQ"}</h2>
    ${signed
      ? `<p>Signed in as <b>${data.user()?.email||"club user"}</b>. Club data is protected by Supabase Row Level Security.</p><div id="accountStatus" class="impStatus"></div><button id="loadClub" class="runImport">TEST CLUB ACCESS</button><button id="signOut" class="accountSecondary">SIGN OUT</button>`
      : `<p>Use a provisioned pilot account to load Maccabi Bnot Ashdod data from the database.</p><div class="importFields"><input id="loginEmail" type="email" autocomplete="username" placeholder="Email"><input id="loginPassword" type="password" autocomplete="current-password" placeholder="Password"></div><div id="accountStatus" class="impStatus"></div><button id="signIn" class="runImport">SIGN IN</button><div class="schema">Pilot accounts are provisioned by CourtIQ. The public demo remains available without database access.</div>`}
  </div>`;
  document.body.appendChild(modal);
  modal.querySelector(".modalX").onclick=()=>modal.remove();
  modal.onclick=e=>{if(e.target===modal)modal.remove()};
  if(signed){
    modal.querySelector("#loadClub").onclick=async()=>{
      const status=modal.querySelector("#accountStatus"); status.textContent="Checking secure club workspace…";
      try{const w=await data.workspace();status.textContent=`Connected · ${w.club.name} · ${w.clubPlayers.length} player records · ${w.games.length} saved games`;}
      catch(e){status.textContent=e.message}
    };
    modal.querySelector("#signOut").onclick=()=>{data.signOut();modal.remove();render();};
  }else{
    modal.querySelector("#signIn").onclick=async()=>{
      const status=modal.querySelector("#accountStatus");
      const email=modal.querySelector("#loginEmail").value.trim(),password=modal.querySelector("#loginPassword").value;
      try{
        if(!email||!password) throw new Error("Enter email and password.");
        status.textContent="Signing in…"; await data.signIn(email,password);
        status.textContent="Connected securely."; modal.remove(); render(); openPilotDashboard();
      }catch(e){status.textContent=e.message}
    };
  }
}
...
function openPilotDashboard(){
  ...
  modal.innerHTML=`<div class="modalCard productHome"><button class="modalX">×</button>
    ...
    <button id="dashboardAccount" class="dashboardAccount">${signedIn?"VERIFY LIVE CLUB DATA":"CONNECT CLUB DATABASE"}</button><div id="dashboardDataStatus" class="impStatus"></div><div class="productTrust"><b>COURTIQ ANALYSIS STANDARD</b><span>DATA → FINDING → INTERPRETATION → VIDEO VERIFICATION</span><small>Box-score evidence never becomes tactical causation without supporting video.</small></div>
  </div>`;
  document.body.appendChild(modal);
  modal.querySelector(".modalX").onclick=()=>modal.remove();
  modal.onclick=e=>{if(e.target===modal)modal.remove()};
  modal.querySelector("#dashboardAccount").onclick=async()=>{if(!window.CourtIQData?.isSignedIn()){modal.remove();openAccount();return;}const status=modal.querySelector("#dashboardDataStatus");status.textContent="Loading secure workspace…";try{const w=await window.CourtIQData.workspace();status.textContent=`LIVE DATABASE · ${w.club.name} · ${w.clubPlayers.length} player records · ${w.games.length} saved games`;}catch(e){status.textContent=e.message}};
...
function localTeamGames(team){
  return Object.values(games).filter(g=>g && (g.home===team||g.away===team));
}
function openOpponentScout(){
  const G=games[active], opponent=G.away, sample=localTeamGames(opponent);
  const modal=document.createElement("div"); modal.className="modal";
  const rows=sample.map(g=>{const isHome=g.home===opponent;return `<tr><td>${g.date}</td><td>${g.home} ${g.hs}–${g.as} ${g.away}</td><td>${isHome?g.hs:g.as}</td><td>${isHome?g.as:g.hs}</td></tr>`}).join("");
  modal.innerHTML=`<div class="modalCard compareModal"><button class="modalX">×</button><small class="eyebrow">COURTIQ · OPPONENT SCOUTING</small><h2>${opponent}</h2><p>Verified scouting workspace based only on games currently stored in CourtIQ.</p><div class="reportKpis"><div><small>SAMPLE</small><b>${sample.length} game${sample.length===1?"":"s"}</b></div><div><small>CONFIDENCE</small><b>${sample.length>=3?"DATA CONFIRMED":"SMALL SAMPLE"}</b></div></div><table class="stats"><tr><th>Date</th><th>Game</th><th>PF</th><th>PA</th></tr>${rows||'<tr><td colspan="4">No stored games for this opponent yet.</td></tr>'}</table><h3>Scouting Priorities</h3><div class="videoList">${G.videos.map((x,i)=>`<div class="videoItem"><span>${i+1}</span>${x}</div>`).join("")}</div><div class="insight warning">CourtIQ does not claim tactical causation from box-score data. Use the investigation list to verify on video.</div></div>`;
  document.body.appendChild(modal); modal.querySelector(".modalX").onclick=()=>modal.remove();
}
function openFullReport(){
  const G=games[active];
  const modal=document.createElement("div"); modal.className="modal";
  modal.innerHTML=`<div class="modalCard compareModal reportModal"><button class="modalX">×</button><small class="eyebrow">COURTIQ · FULL GAME REPORT</small><h2>${G.home} ${G.hs}–${G.as} ${G.away}</h2><p>${G.comp} · ${G.date} · ${G.sourceLabel||"COURTIQ DATA"}</p><div class="reportKpis">${G.metrics.map(x=>`<div><small>${x[0]}</small><b>${x[1]}</b><span>vs ${x[2]}</span></div>`).join("")}</div><h3>Key Findings</h3><div class="findings">${G.findings.map((x,i)=>`<div class="finding"><div class="num">${i+1}</div><b>${x[0]}</b><p>${x[1]}</p><em>→ ${x[2]}</em></div>`).join("")}</div><h3>Team Stats</h3><table class="stats"><tr><th>Metric</th><th>${G.home}</th><th>${G.away}</th></tr>${G.stats.map(x=>`<tr><td>${x[0]}</td><td>${x[1]}</td><td>${x[2]}</td></tr>`).join("")}</table><h3>What Should I Watch?</h3><div class="videoList">${G.videos.map((x,i)=>`<div class="videoItem"><span>${i+1}</span>${x}</div>`).join("")}</div><div class="insight">DATA → FINDING → INTERPRETATION → VIDEO VERIFICATION</div></div>`;
  document.body.appendChild(modal); modal.querySelector(".modalX").onclick=()=>modal.remove();
}
