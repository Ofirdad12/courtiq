...
function playerAnalyticsModule(G){
  ...
  const table=(team,players)=>`<div class="card box playerBox"><h3>${htmlEsc(team)} · Advanced Player Analytics</h3><div class="playerScroll"><table class="stats playerStats"><thead><tr><th>Player</th><th>Role</th><th>MIN</th><th>PTS</th><th>REB</th><th>AST</th><th>eFG%</th><th>TS%</th><th>AST/TO</th><th>PTS/40</th><th>REB/40</th><th>AST/40</th><th>Play-end%</th><th>OREB%</th><th>DREB%</th><th>Box Impact/40</th><th>+/-</th></tr></thead><tbody>${players.map(p=>`<tr><td>${htmlEsc(p.name)}</td><td>${p.starter?"Starter":"Bench"}</td><td>${p.minutes??"—"}</td><td>${p.points??"—"}</td><td>${p.rebounds??"—"}</td><td>${p.ast??"—"}</td><td>${p.efg??"—"}</td><td>${p.ts??"—"}</td><td>${p.ast_to??"—"}</td><td>${p.points_per_40??"—"}</td><td>${p.rebounds_per_40??"—"}</td><td>${p.assists_per_40??"—"}</td><td>${p.play_end_share??"—"}</td><td>${p.oreb_pct??"—"}</td><td>${p.dreb_pct??"—"}</td><td>${p.box_impact_per_40??"—"}</td><td>${p.plus_minus??"—"}</td></tr>`).join("")}</tbody></table></div></div>`;
...
function exportCurrentGame(G,format){
  ...
  const header=["team","player","role","minutes","points","rebounds","assists","turnovers","efg_pct","ts_pct","ast_to","pts_per_40","reb_per_40","ast_per_40","play_end_share_pct","oreb_pct","dreb_pct","box_impact_per_40","plus_minus"];
  const rows=[header,...[[G.home,G.players?.home||[]],[G.away,G.players?.away||[]]].flatMap(([team,players])=>players.map(p=>[team,p.name,p.starter?"starter":"bench",p.minutes,p.points,p.rebounds,p.ast,p.tov,p.efg,p.ts,p.ast_to,p.points_per_40,p.rebounds_per_40,p.assists_per_40,p.play_end_share,p.oreb_pct,p.dreb_pct,p.box_impact_per_40,p.plus_minus]))];
...
document.querySelector(".v2bar b").textContent="MACCABI BNOT ASHDOD · BUILD 102";
